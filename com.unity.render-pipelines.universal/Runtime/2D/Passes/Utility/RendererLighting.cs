...
namespace UnityEngine.Experimental.Rendering.Universal
{
    internal static class RendererLighting
    {
        ...
        public static void RenderLightVolumes(this IRenderPass2D pass, RenderingData renderingData, CommandBuffer cmd, int layerToRender, int endLayerValue, RenderTargetIdentifier renderTexture, RenderTargetIdentifier depthTexture, RenderBufferStoreAction depthStore, List<Light2D> lights)
        {
            var maxShadowTextureCount = ShadowRendering.maxTextureCount;
            var requiresRTInit = true;
            ...
            while (lightIndex < lights.Count)
            {
                ...
                if (shadowLightCount > 0 || requiresRTInit)
                {
                    ...
                    cmd.SetRenderTarget(renderTexture, RenderBufferLoadAction.Load, RenderBufferStoreAction.Store, depthTexture, RenderBufferLoadAction.Load, depthStore);
                    requiresRTInit = false;
                }
